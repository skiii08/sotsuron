...
# ディレクトリパス（適宜変更してください）
# review_per_users用の設定（movie_titleカラムあり）
INPUT_DIR_USERS = "/Users/watanabesaki/PycharmProjects/sotsuron/review_per_movie"
OUTPUT_DIR_USERS = "/Users/watanabesaki/PycharmProjects/sotsuron/review_per_movie/segmented"

# review_per_movie用の設定（usernameカラムあり）
INPUT_DIR_MOVIES = "/Users/watanabesaki/PycharmProjects/sotsuron/review_per_movie"
OUTPUT_DIR_MOVIES = "/Users/watanabesaki/PycharmProjects/sotsuron/review_per_movie/segmented"

# 使用するディレクトリを選択（True: users, False: movies）
USE_USERS_DIR = True  # この値を変更してディレクトリを切り替え

# 実際に使用するパスを設定
if USE_USERS_DIR:
    INPUT_DIR = INPUT_DIR_USERS
    OUTPUT_DIR = OUTPUT_DIR_USERS
    EXPECTED_COLUMNS = ["movie_title", "title", "review", "rating", "helpful", "total", "date"]
    LOG_COLUMN = "movie_title"
else:
    INPUT_DIR = INPUT_DIR_MOVIES
    OUTPUT_DIR = OUTPUT_DIR_MOVIES
    EXPECTED_COLUMNS = ["username", "rating", "helpful", "total", "date", "title", "review"]
    LOG_COLUMN = "username"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile, \
            ...
            for row in tqdm(reader, desc=f"Processing {filename}", total=total_rows):
                try:
                    # ログ用のカラム取得（movie_title または username）
                    log_value = row.get(LOG_COLUMN, 'unknown')
                    ...
                    print(f"\nエラー（行 {processed_reviews + 1}、{LOG_COLUMN}: {log_value}）: {e}")
                    continue
...
def main():
    ...
    # 使用するディレクトリ情報を表示
    if USE_USERS_DIR:
        print("3. 処理対象: review_per_users (movie_titleカラム)")
    else:
        print("3. 処理対象: review_per_movie (usernameカラム)")
    ...
    try:
        ...
        print(f"想定カラム構成: {', '.join(EXPECTED_COLUMNS)}")
